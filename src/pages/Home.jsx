...
export default function Home() {
    ...
    useEffect(() => {
        ...
        socket.onmessage = async (event) => {
            ...
            if (data.START !== "True") {
                ...
                console.log("Tool8value",data.Tool8 )
            ...
            console.log("myreason",datareason);


                if(datareason.Reason==="True")
                {
                    setPopMessage({
                        title:"Reason Adding",
                        message:(
                            <>
                            <button className='btn btn-danger mr-[20px]'
                              onClick={()=>{updateReason;setPopupVisible(false)}}
                              style={{margin:"50px"}}>
                                OK
                              </button>
    ...
    useEffect(() => {
        ...
        socket.onmessage = (event) => {
          const data = JSON.parse(event.data);
          console.log("ourdata",data.ID_Reading);
          if (data.ID_Reading) {
    ...
    useEffect(() => {
        ...
        const fetchReason = async () => {
            ...
            try {
                ...
                console.log("Reason value:", data.results[0].Value);
        // Log the correct value
        
                // Check if the value is "True" correctly
                if (data.results.length > 0 && data.results[0].Value === "True") {
                    setmypopup(true);
                    setShowProgress(false); // Show the popup if the Reason is "True"
                    setPopMessage({
                        title: "Reason Adding",
                        message: (
                            <>
    ...
    const updateReason = async () => {
        console.log("updatereason running");
        try {
            const response = await fetch("http://localhost:3006/updateReason", {
                method: "PUT", // Send POST request to update the reason
            });
            ...
            if (response.ok) {
                ...
                setmypopup(false); // Close the popup after the update is successful
            } else {
